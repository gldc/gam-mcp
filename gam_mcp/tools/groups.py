...
def propose_add_member(
    ...
    if normalised_role not in ("member", "manager", "owner"):
        raise ValueError(f"invalid role {role!r}; must be member, manager, or owner")
    # GAM syntax: gam update group GROUP add [<GroupRole>] USER
    # <GroupRole> is owner|manager|member (member is the default and can be omitted)
    argv = ["gam", "update", "group", group_email, "add"]
    if normalised_role != "member":
        argv.append(normalised_role)
    argv.append(user_email)
    fp = _fingerprint(argv)
    p = store.create_proposal(
        "groups.propose_add_member",
        {"group_email": group_email, "user_email": user_email, "role": normalised_role},
        argv,
        fp,
    ...
    return {
        ...
        "approval_token": p.proposal_id,
        "command_preview": argv,
    }


def propose_remove_member(
    cfg: AppConfig, store: Store, group_email: str, user_email: str
) -> Dict[str, Any]:
    if not is_allowed_email(cfg, user_email):
        raise ValueError("user_email domain not allowlisted")
